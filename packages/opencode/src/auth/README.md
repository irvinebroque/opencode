...
When `webfetch` gets a 401 or 403 response, the orchestration layer in `tool/webfetch.ts` (`handleAuth()`) drives this sequence:
...
4. **Execute the OAuth flow** (`flow.ts`):
   - **Authorization Code + PKCE** ([RFC 6749 &sect;4.1](https://www.rfc-editor.org/rfc/rfc6749.html#section-4.1) + [RFC 7636](https://www.rfc-editor.org/rfc/rfc7636.html)): Starts a local HTTP server on `127.0.0.1:19877`, opens the user's browser to the authorization endpoint, waits for the callback with an authorization code, then exchanges the code (with PKCE verifier) for tokens.
   - **Device Authorization Grant** ([RFC 8628](https://www.rfc-editor.org/rfc/rfc8628.html)): For headless/SSH environments. Displays a `user_code` and `verification_uri` for the user to visit, then polls the token endpoint until authorization completes.
...
sequenceDiagram
    ...
    alt Authorization Code + PKCE (interactive)
        OC->>OC: Start local HTTP server on 127.0.0.1:19877
        ...
        AS->>Browser: Redirect to http://127.0.0.1:19877/webfetch/oauth/callback?code=...&state=...
    ...
    Note over OC,RS: redirect: "manual" to prevent token leakage on cross-origin redirects
...
Includes SSRF protections: loopback metadata URLs from non-loopback resources are rejected.

### `flow.ts`

Executes OAuth flows:

- **Authorization Code + PKCE**: Starts a local callback server on `127.0.0.1:19877` (with port fallback), generates PKCE `code_verifier` + `S256` `code_challenge` per [RFC 7636](https://www.rfc-editor.org/rfc/rfc7636.html), opens the browser, waits for the callback, validates the `state` parameter (CSRF protection), and exchanges the authorization code for tokens. Client registration ([RFC 7591](https://www.rfc-editor.org/rfc/rfc7591.html)) is deferred until after the server binds so the `redirect_uri` port matches.

- **Device Authorization Grant**: For headless/SSH environments per [RFC 8628](https://www.rfc-editor.org/rfc/rfc8628.html). Initiates the device authorization request, returns a `user_code` + `verification_uri` for display, and polls the token endpoint with `slow_down` backoff.
...
Per-origin credential store. Persists credentials as JSON at `$XDG_DATA_HOME/opencode/webfetch-auth.json` (file mode `0600`, directory mode `0700`). Supports `bearer` and `basic` auth schemes. Credential lookup uses exact URL match, then origin match, then longest path-prefix match (path-segment-boundary-aware). Handles token refresh via the `refresh_token` grant ([RFC 6749 &sect;6](https://www.rfc-editor.org/rfc/rfc6749.html#section-6)). Uses an in-memory mutex to serialize concurrent writes and prevent TOCTOU races.
...
The auth orchestration logic lives in `tool/webfetch.ts` (`handleAuth()`), not in this directory. This avoids circular imports between the credential store and the flow executor, and keeps each auth module independently testable. The `handleAuth()` function ties together: challenge parsing, discovery, user consent prompt, flow selection (auth code vs. device code), credential storage, and retry with credentials.
